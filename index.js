...
app.get('/registration', notAuth, (req, res) => {
...
app.get('/host/profile', isAuth, (req, res) => {
    ...
    .then(host => {
        res.render('profile', {data: host.hostName})
    })
    .catch(err => console.log(err))
})
app.post('/host/addinfo', isAuth, (req, res) => {
    Host.findOne({_id: req.session.hostID})
    .then(host => {
        host.infos.room = req.body.room
        host.infos.from = req.body.from
        host.infos.to = req.body.to
        host.infos.price = req.body.price
        host.save()
...
app.post('/host/accept', (req, res) => {
    ...
    .then(host => {
        ...
        .then(user => {
            ...
            // sub rooms
            host.infos.room = host.infos.room - parseInt(userRequest.rooms)
            // add to notification
            ...
            host.requests = host.requests.filter(r => r.by !== userRequest.by)
